...
local tempo_and_pitch_distort = function(m)
    local tempoMult = 1
    local pitchAdd = 0
    local lowHpDistort = gSGELocalSettings.lowHpMusic or 0
    ...
    if mario_is_crouching(m) and m.action ~= ACT_START_CROUCHING then
        tempoMult = tempoMult * (1 - (gSGELocalSettings.crouchSlowMusic or 0))
    end
        ...
        if sLookObj then
            if not obj_within_looking_range(m, sLookObj) or not obj_is_visible(sLookObj) then
                ...
                if get_id_from_behavior(sLookObj.behavior) == id_bhvMario then
                    lookPoint.y = gMarioStates[0].marioBodyState.headPos.y
                end
...
hook_event(HOOK_BEFORE_SET_MARIO_ACTION, function(m, nextAct)
    ...
    local customDeaths = (gSGELocalSettings.deathScene or 0) <= 2
...
hook_event(HOOK_ON_INTERACT, function(m, o)
    ...
    if o.oInteractType == INTERACT_STAR_OR_KEY then
        ...
        if not is_very_important(o) and gServerSettings and gServerSettings.stayInLevelAfterStar == 2 and gSGELocalSettings.miscThings then
