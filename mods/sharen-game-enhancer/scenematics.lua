...
local handle_death_text = function()
    ...
    local inAbyss = (m.action == ACT_INTO_ABYSS)
    local bubba

    -- Death plane deaths (ACT_INTO_ABYSS) are extremely sensitive in singleplayer.
    -- Bypass the Too Bad sequence entirely so the engine can do its normal death warp.
    if inAbyss then
    ...
    if (switch(m.action, gDeathActs, m) or (m.action == ACT_SQUISHED and m.actionTimer >= 30)) and not canBubble and tooBad then
        ...
        if gDeathTextTimer >= DEATH_TEXT_DURATION and m.action ~= ACT_FROZEN_WATER and m.action ~= ACT_BURNT then
...
local run_custom_cutscenes = function()
    local m = gMarioStates[0]
    if m == nil or m.area == nil or m.area.camera == nil then return end
...
hook_event(HOOK_ON_DEATH, function(m)
    ...
    if m.action ~= ACT_INTO_ABYSS and gDeathTextTimer < DEATH_TEXT_DURATION and (gDeathActs[m.action] ~= nil or m.action == ACT_SQUISHED or m.action == ACT_LAVA_BOOST) and
    ((not canBubble and (gSGELocalSettings.deathScene or 0) <= 2)) then
        return false
    end
end)
